...
let
  ...
  importOrCall = path: args:
    let module = import path;
    in if builtins.isFunction module then module args else module;
        ...
        let
          ...
          level2 = if hasMore then
            unionFor (findFiles (hasPostfix "nix") morePath) (fname:
              let result = importOrCall fname { inherit lib; };
          ...
          more =
            ...
            if builtins.pathExists moreMorePath then
              unionFor (findFiles (hasPostfix "nix") moreMorePath) (path: {
                "${builtins.replaceStrings [".nix"] [""] (builtins.baseNameOf path)}" = importOrCall path { inherit lib pkgs; };
              })
            else {};
        };
    };
